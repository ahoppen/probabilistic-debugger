...
class WPInferenceEngineTests: XCTestCase {
  func testInferDeterministicStraightLineProgram() {
    ...
    XCTAssertEqual(inferenceEngine.infer(term: .equal(lhs: .variable(var1), rhs: .integer(2))), .bool(true))
    XCTAssertEqual(inferenceEngine.infer(term: .equal(lhs: .variable(var1), rhs: .integer(1))), .bool(false))
  }
}
